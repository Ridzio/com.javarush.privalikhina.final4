...
public class Main {
    ...
    // Ниже в вашем классе Main должны быть методы prepareRelationalDb() и prepareRedisClient()
    ...
    private void pushToRedis(List<CityCountry> data) {
        try (io.lettuce.core.api.StatefulRedisConnection<String, String> connection = redisClient.connect()) {
            io.lettuce.core.api.sync.RedisStringCommands<String, String> sync = connection.sync();
            for (CityCountry dataItem : data) {
                try {
                    sync.set(String.valueOf(dataItem.getId()), mapper.writeValueAsString(dataItem));
